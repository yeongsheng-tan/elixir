defmodule Election do
  ...
  def update(election, ["a" <> _ | args]) do
    ...
    %{election | candidates: candidates, next_id: election.next_id + 1, errors: ""}
  end

  def update(election, ["n" <> _ | args]) do
    name = Enum.join(args, " ")
    %{election |  name: name, errors: ""}
  end
  ...
  def update(election, _) do
    Map.put(election, :errors, "Invalid command!")
  end

  defp vote(election, {id, ""}) do
    candidates = Enum.map(election.candidates, &maybe_inc_vote(&1, id))
    %{election| candidates: candidates, errors: ""}
  end

  defp vote(election, errors) do
    Map.put(election, :errors, errors)
  end
